...
for trait, pseudobulk in pseudobulks.items():
    if 'SEAAD' in trait:
        pseudobulk.obs['Dx'] = \
            np.where(pseudobulk.obs['Consensus Clinical Dx (choice=Alzheimers disease)'].eq('Checked'), 0,
            np.where(pseudobulk.obs['Consensus Clinical Dx (choice=Control)'].eq('Checked'), 1, np.nan)).astype(float)
